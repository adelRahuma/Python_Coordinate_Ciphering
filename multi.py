...
def process_1(df, random_numbers, random_nums, result_queue_1):
    ...
    result_queue_1.put(df)
    return df


# def process_2(df, result_queue_2):
#     df['x1'] = df['x'].iloc[random_nums].values
#     df.drop('x', axis=1, inplace=True)
#     df['index_x'] = random_nums
#     result_queue_2.put(df)
#     return df
...
p1 = multiprocessing.Process(target=process_1, args=[
                             num_polygons, random_numbers, random_nums, result_queue_1]).start()
# p2 = multiprocessing.Process(target=process_2, args=[
#                              num_polygons, random_nums, result_queue_2]).start()
...
# result1 = result_queue_1.get()

cc = result_queue_1.get()
print(cc)
cc.to_csv(file, sep=',', index=False)
# dd = result_queue_2.get()
# # result2 = result_queue_2.get()

# dd.to_csv(file, sep=',', index=False)
